    ...
    async def execute_tool_call(self, call: types.FunctionCall) -> dict[str, Any]:
        ...
        if tool is None:
            execution_result = ToolExecutionResult(
                model_response={"response": f"Unknown tool: {call.name}"}
            )
            return {
                "name": call.name,
        ...
        if call.args is None:
            execution_result = ToolExecutionResult(
                model_response={
                    "response": f"Tool call '{call.name}' did not include arguments."
                }
            )
            return {
                "name": call.name,
        ...
        try:
            ...
            execution_result = ToolExecutionResult(
                model_response={
                    "response": (f"Invalid arguments for tool '{call.name}':\n{error}")
                }
            )
            return {
                "name": call.name,
        ...
        while True:
            ...
            for attempt in range(5):
                try:
                    ...
                    is_503 = "503" in exc_str or "UNAVAILABLE" in exc_str
                    if (not is_429 and not is_503) or attempt == 4:
                        raise
                    # Parse suggested retry delay from the error message.
                    match = re.search(r"retry[^0-9]*(\d+)", exc_str, re.IGNORECASE)
                    if is_429:
                        delay = int(match.group(1)) + 2 if match else 60
                        print(f"[yellow]Rate limited (429) — retrying in {delay}s...[/yellow]")
                    else:
                        delay = 30 * (attempt + 1)  # 30 s, 60 s, 90 s, 120 s back-off
                        print(f"[yellow]Service unavailable (503) — retrying in {delay}s (attempt {attempt + 1}/5)...[/yellow]")
